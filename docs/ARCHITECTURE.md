...
|-----------|-------|--------------|---------------|---------|
| `qaforge_frontend` | React 18 + nginx | 443 | `${FRONTEND_PORT:-8080}` | SPA + reverse proxy |
| `qaforge_backend` | FastAPI (Python 3.11) | 8000 | *internal only* | REST API + WebSocket |
...
All services include health checks. Backend depends on db + redis + chromadb (healthy). Frontend depends on backend (healthy).
